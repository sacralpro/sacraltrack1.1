import type { NextPage } from "next";
import LeftSideBar from "../components/left-side-bar";
import ContainerTrackListForm from "../components/container-track-list-form";
import Audio1 from "../components/audio1";

const SacralTrackFinalDesignCo1: NextPage = () => {
  return (
    <div className="relative [background:linear-gradient(250.97deg,_#313451,_#0e0929)] w-full h-[632px] overflow-hidden text-left text-3xs text-white font-gotham-pro">
      <div className="absolute top-[30px] left-[0px] w-[889px] flex flex-col items-center justify-center gap-[20px]">
        <div className="rounded-[12.34px] bg-gray-400 box-border w-[848px] flex flex-row py-1.5 px-3 items-center justify-between border-[0.2px] border-solid border-gray-600">
          <div className="relative w-[83.6px] h-3">
            <img
              className="absolute h-[80.28%] w-[calc(100%_-_71.26px)] top-[7.16%] right-[71.26px] bottom-[12.57%] left-[0px] max-w-full overflow-hidden max-h-full"
              alt=""
              src="/logo1.svg"
            />
            <b className="absolute top-[0%] left-[21.6px] leading-[11.73px]">
              Sacral Track
            </b>
          </div>
          <div className="flex flex-row items-center justify-start gap-[36px]">
            <div className="flex flex-row items-center justify-start gap-[10px] font-gotham-pro-narrow">
              <img
                className="relative w-4 h-4 overflow-hidden shrink-0"
                alt=""
                src="/icon-componentsvg.svg"
              />
              <div className="relative leading-[12px] font-medium">Hip-Hop</div>
            </div>
            <div className="relative leading-[7.41px]">People</div>
            <div className="relative leading-[7.41px]">News</div>
            <div className="relative leading-[7.41px]">Events</div>
          </div>
          <div className="flex flex-row items-start justify-start gap-[19px] text-5xs text-mediumseagreen-100">
            <img
              className="relative w-[82.09px] h-[30.86px]"
              alt=""
              src="/button--search1.svg"
            />
            <div className="relative w-[61.72px] h-[30.86px]">
              <img
                className="absolute h-full w-full top-[0%] right-[0%] bottom-[0%] left-[0%] max-w-full overflow-hidden max-h-full"
                alt=""
                src="/rectangle-388.svg"
              />
              <div className="absolute top-[30.76%] left-[10%] leading-[9.88px] font-medium">
                <span>$</span>
                <span className="text-white">10,8</span>
              </div>
              <img
                className="absolute h-full w-6/12 top-[0%] right-[0%] bottom-[0%] left-[50%] max-w-full overflow-hidden max-h-full"
                alt=""
                src="/button--cart1.svg"
              />
            </div>
          </div>
        </div>
        <div className="w-[849px] flex flex-row items-center justify-start gap-[20px]">
          <LeftSideBar
            dimensions="/rectangle-3921.svg"
            productDimensions="/icon2.svg"
            productDimensionsText="/icon--logout2.svg"
            leftSideBarZIndex="unset"
            groupDivCursor="unset"
            groupDivBorder="unset"
            groupDivPadding="unset"
            groupDivBackgroundColor="unset"
            releaseATrackDisplay="inline-block"
          />
          <ContainerTrackListForm
            dimension="/union2.svg"
            productDimensions="/logotype2.svg"
            dimensionText="/union3.svg"
            packageDimensions="/share2.svg"
            productDimensionsText="/logotype.svg"
            artistNameFlexoZIndex="unset"
          />
          <div className="h-[508px] flex flex-col items-start justify-start gap-[20px]">
            <div className="rounded-mini bg-gray-400 w-[244px] flex flex-col p-[5px] box-border items-center justify-start gap-[20px]">
              <div className="flex flex-col items-center justify-start gap-[10px]">
                <div className="relative w-[228px] h-3">
                  <div className="absolute top-[-1px] left-[95px] leading-[20px] font-medium">
                    Top100
                  </div>
                </div>
                <img
                  className="relative w-[234px] h-[0.2px]"
                  alt=""
                  src="/vector-193.svg"
                />
              </div>
              <div className="w-[234px] flex flex-col items-start justify-start gap-[10px] text-[11.77px]">
                <div className="relative w-[234px] h-[76px] text-center text-[10.09px]">
                  <div className="absolute h-[99.57%] w-[101.66%] top-[0.16%] right-[-1.3%] bottom-[0.28%] left-[-0.37%] rounded-[12.61px] bg-gray-500" />
                  <div className="absolute top-[0.12px] left-[198px] w-9 h-[75.67px]">
                    <img
                      className="absolute h-full w-full top-[0%] right-[0%] bottom-[0%] left-[0%] max-w-full overflow-hidden max-h-full opacity-[0.6]"
                      alt=""
                      src="/rectangle-8.svg"
                    />
                    <div className="absolute top-[10.93px] left-[11.27px] flex flex-col items-start justify-start gap-[13px]">
                      <img
                        className="relative w-[13.4px] h-[13.45px] overflow-hidden shrink-0"
                        alt=""
                        src="/icon--more3.svg"
                      />
                      <img
                        className="relative w-[12.19px] h-[0.84px]"
                        alt=""
                        src="/vector-1923.svg"
                      />
                      <img
                        className="relative w-[13.45px] h-[13.45px] overflow-hidden shrink-0"
                        alt=""
                        src="/icon--cart3.svg"
                      />
                    </div>
                  </div>
                  <div className="absolute h-[99.57%] w-[18.28%] top-[0.16%] right-[82.09%] bottom-[0.28%] left-[-0.36%] font-open-sans">
                    <img
                      className="absolute h-full w-[142%] top-[0%] right-[-42%] bottom-[0%] left-[0%] rounded-[12.61px] max-w-full overflow-hidden max-h-full object-cover"
                      alt=""
                      src="/jaanusjagomagi1489986unsplash@2x.png"
                    />
                    <b className="absolute w-[14.27%] top-[40%] left-[42%] flex items-center justify-center">
                      1
                    </b>
                  </div>
                  <div className="absolute top-[12.73px] left-[49.62px] flex flex-col items-start justify-start gap-[8px] text-left text-[11.77px]">
                    <div className="relative w-[54.78px] h-[37.54px]">
                      <b className="absolute w-[63.15%] top-[0%] left-[0%] leading-[15.13px] flex items-center">
                        Artist
                      </b>
                      <div className="absolute w-[98.44%] top-[62.71%] left-[1.56%] text-[10.09px] leading-[13.45px] flex items-center">
                        Thank You
                      </div>
                    </div>
                    <div className="relative w-[55.61px] h-[13.45px] text-[10.09px] text-lightgray">
                      <div className="absolute w-[67.69%] top-[25%] left-[32.31%] flex items-center">
                        1001101
                      </div>
                      <img
                        className="absolute h-full w-[24.61%] top-[0%] right-[75.39%] bottom-[0%] left-[0%] max-w-full overflow-hidden max-h-full"
                        alt=""
                        src="/icon--like3.svg"
                      />
                    </div>
                  </div>
                  <div className="absolute h-[18.42%] w-[11.3%] top-[78.7%] right-[30.2%] bottom-[2.87%] left-[58.49%] text-mediumseagreen-200">
                    <div className="absolute w-full top-[0%] left-[0%] leading-[13.45px] font-medium flex items-center">
                      <span className="[line-break:anywhere] w-full">
                        <span>$</span>
                        <span className="text-white">5.18</span>
                      </span>
                    </div>
                  </div>
                </div>
                <div className="relative shadow-[0px_0px_8.41px_rgba(0,_0,_0,_0.2)] w-[234px] h-[76px]">
                  <div className="absolute top-[0.14px] left-[0px] w-[234px] h-[76px]">
                    <div className="absolute top-[0px] left-[0px] w-[234px] h-[76px]">
                      <div className="absolute h-full w-[90.35%] top-[0%] right-[9.65%] bottom-[0%] left-[0%] rounded-[12.61px] bg-gray-500" />
                      <div className="absolute top-[0px] left-[197.85px] w-[36.15px] h-[75.67px]">
                        <img
                          className="absolute h-full w-[99.57%] top-[0%] right-[0.43%] bottom-[0%] left-[0%] max-w-full overflow-hidden max-h-full opacity-[0.6]"
                          alt=""
                          src="/rectangle-81.svg"
                        />
                        <div className="absolute top-[10.93px] left-[11.27px] flex flex-col items-start justify-start gap-[13px]">
                          <img
                            className="relative w-[13.45px] h-[13.45px] overflow-hidden shrink-0"
                            alt=""
                            src="/icon--more4.svg"
                          />
                          <img
                            className="relative w-[12.19px] h-[0.84px]"
                            alt=""
                            src="/vector-1924.svg"
                          />
                          <img
                            className="relative w-[13.45px] h-[13.45px] overflow-hidden shrink-0"
                            alt=""
                            src="/icon--cart4.svg"
                          />
                        </div>
                      </div>
                      <img
                        className="absolute h-full w-[18.44%] top-[0%] right-[81.56%] bottom-[0%] left-[0%] max-w-full overflow-hidden max-h-full object-cover"
                        alt=""
                        src="/cover@2x.png"
                      />
                      <div className="absolute top-[12.67px] left-[50.92px] flex flex-col items-start justify-start gap-[8px]">
                        <div className="relative w-[55.26px] h-[37.7px]">
                          <b className="absolute h-[42.62%] w-[63.15%] top-[0%] left-[0%] leading-[15.13px] flex items-center">
                            Artist
                          </b>
                          <div className="absolute h-[37.29%] w-[98.44%] top-[62.71%] left-[1.56%] text-[10.09px] leading-[13.45px] flex items-center">
                            Thank You
                          </div>
                        </div>
                        <div className="relative w-[54.04px] h-[13.51px] text-[9.76px] text-gray-100">
                          <div className="absolute h-[74.33%] w-[66.47%] top-[25%] left-[33.53%] flex items-center">
                            1001101
                          </div>
                          <img
                            className="absolute h-full w-[25.55%] top-[0%] right-[74.45%] bottom-[0%] left-[0%] max-w-full overflow-hidden max-h-full"
                            alt=""
                            src="/icon--like4.svg"
                          />
                        </div>
                      </div>
                      <div className="absolute h-[18.5%] w-[11.4%] top-[78.89%] right-[29.22%] bottom-[2.61%] left-[59.37%] text-center text-[10.09px] text-mediumseagreen-200">
                        <div className="absolute h-full w-full top-[0%] left-[0%] leading-[13.45px] font-medium flex items-center">
                          <span className="[line-break:anywhere] w-full">
                            <span>$</span>
                            <span className="text-white">5.18</span>
                          </span>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="relative shadow-[0px_0px_8.41px_rgba(0,_0,_0,_0.2)] w-[234px] h-[76px]">
                  <div className="absolute top-[0px] left-[0px] w-[234px] h-[76px]">
                    <div className="absolute top-[0px] left-[0px] w-[234px] h-[76px]">
                      <div className="absolute h-full w-full top-[0%] right-[0%] bottom-[0%] left-[0%] rounded-[12.61px] bg-gray-500" />
                      <img
                        className="absolute h-full w-[15.38%] top-[0%] right-[0.17%] bottom-[0%] left-[84.45%] max-w-full overflow-hidden max-h-full opacity-[0.6]"
                        alt=""
                        src="/bgbutton.svg"
                      />
                      <div className="absolute top-[10.98px] left-[207.97px] flex flex-col items-center justify-start gap-[13px]">
                        <img
                          className="relative w-[15.28px] h-[13.51px] overflow-hidden shrink-0"
                          alt=""
                          src="/icon--more2.svg"
                        />
                        <img
                          className="relative w-[12.19px] h-[0.84px]"
                          alt=""
                          src="/vector-1922.svg"
                        />
                        <img
                          className="relative w-[13.45px] h-[13.45px] overflow-hidden shrink-0"
                          alt=""
                          src="/icon--cart5.svg"
                        />
                      </div>
                      <img
                        className="absolute h-full w-[20.41%] top-[0%] right-[79.59%] bottom-[0%] left-[0%] max-w-full overflow-hidden max-h-full object-cover"
                        alt=""
                        src="/cover1@2x.png"
                      />
                      <div className="absolute top-[12.67px] left-[56.35px] flex flex-col items-start justify-start gap-[8px]">
                        <div className="relative w-[61.16px] h-[37.7px]">
                          <b className="absolute h-[42.62%] w-[63.15%] top-[0%] left-[0%] leading-[15.13px] flex items-center">
                            Artist
                          </b>
                          <div className="absolute h-[37.29%] w-[98.44%] top-[62.71%] left-[1.56%] text-[10.09px] leading-[13.45px] flex items-center">
                            Thank You
                          </div>
                        </div>
                        <div className="relative w-[59.81px] h-[13.51px] text-[9.76px] text-gray-100">
                          <div className="absolute h-[74.33%] w-[66.47%] top-[25%] left-[33.53%] flex items-center">
                            1001101
                          </div>
                          <img
                            className="absolute h-full w-[25.55%] top-[0%] right-[74.45%] bottom-[0%] left-[0%] max-w-full overflow-hidden max-h-full"
                            alt=""
                            src="/icon--like2.svg"
                          />
                        </div>
                      </div>
                      <div className="absolute h-[18.5%] w-[12.62%] top-[78.89%] right-[21.66%] bottom-[2.61%] left-[65.71%] text-center text-[10.09px] text-mediumseagreen-200">
                        <div className="absolute h-full w-full top-[0%] left-[0%] leading-[13.45px] font-medium flex items-center">
                          <span className="[line-break:anywhere] w-full">
                            <span>$</span>
                            <span className="text-white">5.18</span>
                          </span>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="relative rounded-3xs bg-white w-6 h-0.5" />
            </div>
            <div className="rounded-mini bg-gray-400 w-[244px] h-[168px] flex flex-col p-[5px] box-border items-start justify-between text-[8.56px]">
              <div className="w-[234px] flex flex-row items-start justify-start gap-[62px]">
                <div className="w-[156px] flex flex-row items-end justify-start gap-[3px]">
                  <img
                    className="relative w-[35.8px] h-[37.03px] object-cover"
                    alt=""
                    src="/cover2@2x.png"
                  />
                  <div className="flex flex-col items-start justify-start gap-[4px]">
                    <b className="relative leading-[9.26px] flex items-center w-[90.73px] h-[9.88px] shrink-0">
                      Artist
                    </b>
                    <div className="relative text-[7.34px] leading-[9.79px] flex items-center w-[93.2px] h-[19.75px] shrink-0">
                      <span className="[line-break:anywhere] w-full">
                        <p className="m-0 tracking-[0.05em] font-medium">{`You are amaizing! `}</p>
                        <p className="m-0">(Remix by Artist)</p>
                      </span>
                    </div>
                  </div>
                </div>
                <div className="relative w-3 h-3 overflow-hidden shrink-0">
                  <div className="absolute top-[4.5px] left-[0px] w-3 h-[3px]">
                    <div className="absolute top-[0px] left-[0px] w-3 flex flex-row items-start justify-start gap-[1px]">
                      <div className="relative rounded-[50%] bg-white w-[3px] h-[3px]" />
                      <div className="relative rounded-[50%] bg-white w-[3px] h-[3px]" />
                      <div className="relative rounded-[50%] bg-white w-[3px] h-[3px]" />
                    </div>
                  </div>
                </div>
              </div>
              <div className="bg-gray-200 w-[234px] h-11 flex flex-col py-[11px] px-0.5 box-border items-start justify-center">
                <Audio1
                  audioWidth="230px"
                  audioHeight="13px"
                  rectangleDivBorderRadius="2.86px"
                  rectangleDivBorderRadius1="2.86px"
                  rectangleDivBorderRadius2="2.86px"
                  rectangleDivBorderRadius3="2.86px"
                  rectangleDivBorderRadius4="2.86px"
                  rectangleDivBorderRadius5="2.86px"
                  rectangleDivBorderRadius6="2.86px"
                  rectangleDivBorderRadius7="2.86px"
                  rectangleDivBorderRadius8="2.86px"
                  rectangleDivBorderRadius9="2.86px"
                  rectangleDivBorderRadius10="2.86px"
                  rectangleDivBorderRadius11="2.86px"
                  rectangleDivBorderRadius12="2.86px"
                  rectangleDivBorderRadius13="2.86px"
                  rectangleDivBorderRadius14="2.86px"
                  rectangleDivBorderRadius15="2.86px"
                  rectangleDivBorderRadius16="2.86px"
                  rectangleDivBorderRadius17="2.86px"
                  rectangleDivBorderRadius18="2.86px"
                  rectangleDivBorderRadius19="2.86px"
                  rectangleDivBorderRadius20="2.86px"
                  rectangleDivBorderRadius21="2.86px"
                  rectangleDivBorderRadius22="2.86px"
                  rectangleDivBorderRadius23="2.86px"
                  rectangleDivBorderRadius24="2.86px"
                  rectangleDivBorderRadius25="2.86px"
                  rectangleDivBorderRadius26="2.86px"
                  rectangleDivBorderRadius27="2.86px"
                  rectangleDivBorderRadius28="2.86px"
                  rectangleDivBorderRadius29="2.86px"
                  rectangleDivBorderRadius30="2.86px"
                  rectangleDivBorderRadius31="2.86px"
                  rectangleDivBorderRadius32="2.86px"
                  rectangleDivBorderRadius33="2.86px"
                  rectangleDivBorderRadius34="2.86px"
                  rectangleDivBorderRadius35="2.86px"
                  rectangleDivBorderRadius36="2.86px"
                  rectangleDivBorder="unset"
                  rectangleDivBackgroundColor="unset"
                  rectangleDivBorderRadius37="2.86px"
                  rectangleDivBorderRadius38="2.86px"
                  rectangleDivBorderRadius39="2.86px"
                  rectangleDivBorderRadius40="2.86px"
                  rectangleDivBorderRadius41="2.86px"
                  rectangleDivBorderRadius42="2.86px"
                  rectangleDivBorderRadius43="2.86px"
                  rectangleDivBorderRadius44="2.86px"
                  rectangleDivBorderRadius45="2.86px"
                  rectangleDivBorderRadius46="2.86px"
                  rectangleDivBorderRadius47="2.86px"
                  rectangleDivBorderRadius48="2.86px"
                  rectangleDivBorderRadius49="2.86px"
                  rectangleDivBorderRadius50="2.86px"
                  rectangleDivBorderRadius51="2.86px"
                  rectangleDivBorderRadius52="2.86px"
                  rectangleDivBorderRadius53="2.86px"
                  rectangleDivBorderRadius54="2.86px"
                  rectangleDivBorderRadius55="2.86px"
                  rectangleDivBorderRadius56="2.86px"
                  rectangleDivBorderRadius57="2.86px"
                  rectangleDivLeft="42.11%"
                  rectangleDivBorderRadius58="2.86px"
                  rectangleDivBorderRadius59="2.86px"
                  rectangleDivBorderRadius60="2.86px"
                  rectangleDivTop="39.28%"
                  rectangleDivBorderRadius61="2.86px"
                  rectangleDivTop1="39.28%"
                  rectangleDivBorderRadius62="2.86px"
                  rectangleDivBorderRadius63="2.86px"
                  rectangleDivBorderRadius64="2.86px"
                  rectangleDivBorderRadius65="2.86px"
                  rectangleDivBorderRadius66="2.86px"
                  rectangleDivBottom="10.72%"
                  rectangleDivBorderRadius67="2.86px"
                  rectangleDivBottom1="10.72%"
                  rectangleDivBorderRadius68="2.86px"
                  rectangleDivBorderRadius69="2.86px"
                  rectangleDivBorderRadius70="2.86px"
                  rectangleDivTop2="39.28%"
                  rectangleDivBorderRadius71="2.86px"
                  rectangleDivTop3="39.28%"
                  rectangleDivBorderRadius72="2.86px"
                  rectangleDivBorderRadius73="2.86px"
                  rectangleDivBorderRadius74="2.86px"
                  rectangleDivBorderRadius75="2.86px"
                  rectangleDivBorderRadius76="2.86px"
                  rectangleDivBottom2="10.72%"
                  rectangleDivRight="26.32%"
                  rectangleDivBorderRadius77="2.86px"
                  rectangleDivBottom3="10.72%"
                  rectangleDivBorderRadius78="2.86px"
                  rectangleDivLeft1="57.89%"
                  rectangleDivBorderRadius79="2.86px"
                  rectangleDivBorderRadius80="2.86px"
                  rectangleDivTop4="39.28%"
                  rectangleDivBorderRadius81="2.86px"
                  rectangleDivTop5="39.28%"
                  rectangleDivBorderRadius82="2.86px"
                  rectangleDivBorderRadius83="2.86px"
                  rectangleDivBorderRadius84="2.86px"
                  rectangleDivBorderRadius85="2.86px"
                  rectangleDivBorderRadius86="2.86px"
                  rectangleDivBottom4="10.72%"
                  rectangleDivRight1="26.32%"
                  rectangleDivBorderRadius87="2.86px"
                  rectangleDivBottom5="10.72%"
                  rectangleDivBorderRadius88="2.86px"
                  rectangleDivLeft2="57.89%"
                  rectangleDivBorderRadius89="2.86px"
                  rectangleDivBorderRadius90="2.86px"
                  rectangleDivBorderRadius91="2.86px"
                  rectangleDivBorderRadius92="2.86px"
                  rectangleDivBorderRadius93="2.86px"
                  rectangleDivBorderRadius94="2.86px"
                  rectangleDivBorderRadius95="2.86px"
                  rectangleDivBorderRadius96="2.86px"
                  rectangleDivBorderRadius97="2.86px"
                  rectangleDivBorderRadius98="2.86px"
                  rectangleDivBorderRadius99="2.86px"
                  rectangleDivBorderRadius100="2.86px"
                  rectangleDivBorderRadius101="2.86px"
                  rectangleDivBorderRadius102="2.86px"
                  rectangleDivBorderRadius103="2.86px"
                  rectangleDivBorderRadius104="2.86px"
                  rectangleDivBorderRadius105="2.86px"
                  rectangleDivBorderRadius106="2.86px"
                  rectangleDivBorderRadius107="2.86px"
                  rectangleDivBorderRadius108="2.86px"
                  rectangleDivBorderRadius109="2.86px"
                  rectangleDivBorderRadius110="2.86px"
                  rectangleDivBorderRadius111="2.86px"
                  rectangleDivBorderRadius112="2.86px"
                  rectangleDivBorderRadius113="2.86px"
                  rectangleDivBorderRadius114="2.86px"
                  rectangleDivBorderRadius115="2.86px"
                  rectangleDivBorderRadius116="2.86px"
                  rectangleDivBorderRadius117="2.86px"
                  rectangleDivLeft3="42.11%"
                  rectangleDivBorderRadius118="2.86px"
                  rectangleDivBorderRadius119="2.86px"
                  rectangleDivBorderRadius120="2.86px"
                  rectangleDivBorderRadius121="2.86px"
                  rectangleDivBorderRadius122="2.86px"
                  rectangleDivBorderRadius123="2.86px"
                  rectangleDivRight2="73.68%"
                  rectangleDivBorderRadius124="2.86px"
                  rectangleDivBorderRadius125="2.86px"
                  rectangleDivBorderRadius126="2.86px"
                  rectangleDivBorderRadius127="2.86px"
                  rectangleDivLeft4="42.11%"
                  rectangleDivBorderRadius128="2.86px"
                  rectangleDivBorderRadius129="2.86px"
                  rectangleDivBorderRadius130="2.86px"
                  rectangleDivBorderRadius131="2.86px"
                  rectangleDivBorderRadius132="2.86px"
                  rectangleDivBorderRadius133="2.86px"
                  rectangleDivBorderRadius134="2.86px"
                  rectangleDivBorderRadius135="2.86px"
                  rectangleDivBorderRadius136="2.86px"
                  rectangleDivBorderRadius137="2.86px"
                  rectangleDivLeft5="42.11%"
                  rectangleDivBorderRadius138="2.86px"
                  rectangleDivBorderRadius139="2.86px"
                  rectangleDivBorderRadius140="2.86px"
                  rectangleDivBorderRadius141="2.86px"
                  rectangleDivBorderRadius142="2.86px"
                  rectangleDivBorderRadius143="2.86px"
                  rectangleDivBorderRadius144="2.86px"
                  rectangleDivBorderRadius145="2.86px"
                  rectangleDivBorderRadius146="2.86px"
                  rectangleDivBorderRadius147="2.86px"
                  rectangleDivBorderRadius148="2.86px"
                  rectangleDivBorderRadius149="2.86px"
                  rectangleDivBorderRadius150="2.86px"
                  rectangleDivBorderRadius151="2.86px"
                  rectangleDivBorder1="unset"
                  rectangleDivBackgroundColor1="unset"
                  rectangleDivBorderRadius152="2.86px"
                  rectangleDivBorderRadius153="2.86px"
                  rectangleDivRight3="73.68%"
                  rectangleDivBorderRadius154="2.86px"
                  rectangleDivBorderRadius155="2.86px"
                  rectangleDivBorderRadius156="2.86px"
                  rectangleDivBorderRadius157="2.86px"
                  rectangleDivLeft6="42.11%"
                  rectangleDivBorderRadius158="2.86px"
                  rectangleDivBorderRadius159="2.86px"
                  rectangleDivBorderRadius160="2.86px"
                  rectangleDivBorder2="unset"
                  rectangleDivBackgroundColor2="unset"
                  rectangleDivBorderRadius161="2.86px"
                  rectangleDivBorderRadius162="2.86px"
                  rectangleDivBorder3="unset"
                  rectangleDivBackgroundColor3="unset"
                  rectangleDivBorderRadius163="2.86px"
                  rectangleDivBorderRadius164="2.86px"
                  rectangleDivBorderRadius165="2.86px"
                  rectangleDivBorderRadius166="2.86px"
                  rectangleDivBorderRadius167="2.86px"
                  rectangleDivBorderRadius168="2.86px"
                  rectangleDivLeft7="57.89%"
                  rectangleDivBorderRadius169="2.86px"
                />
              </div>
              <div className="w-[233px] flex flex-row items-start justify-between text-5xs">
                <div className="relative w-[50px] h-10">
                  <img
                    className="absolute h-full w-full top-[0%] right-[0%] bottom-[0%] left-[0%] rounded-3xs max-w-full overflow-hidden max-h-full"
                    alt=""
                    src="/rectangle-3961.svg"
                  />
                  <div className="absolute h-[30%] w-[24%] top-[34.84%] right-[38%] bottom-[35.16%] left-[38%] overflow-hidden">
                    <div className="absolute top-[1.06px] left-[1.4px] flex flex-row items-start justify-start gap-[4px]">
                      <div className="relative rounded-sm bg-white w-[2.6px] h-2.5" />
                      <div className="relative rounded-sm bg-white w-[2.6px] h-2.5" />
                    </div>
                  </div>
                </div>
                <div className="relative w-[120px] h-10">
                  <div className="absolute top-[0px] left-[0px] rounded-3xs bg-gray-200 w-[120px] h-10" />
                  <div className="absolute top-[0px] left-[40.17px] h-10 flex flex-col items-center justify-center">
                    <div className="flex flex-col items-center justify-start gap-[8px]">
                      <img
                        className="relative w-[13px] h-[13px] overflow-hidden shrink-0"
                        alt=""
                        src="/icon--tracklist1.svg"
                      />
                      <div className="relative leading-[8px] font-medium">
                        PLAYLIST
                      </div>
                    </div>
                  </div>
                </div>
                <img
                  className="relative w-10 h-10"
                  alt=""
                  src="/button--same.svg"
                />
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default SacralTrackFinalDesignCo1;
